...
export function useBooks() {
  ...
  const updateProgress = async (id, currentPage) => {
    ...
    await updateBook(id, {
      ...
      finished_date: finished ? toDateStr(new Date()) : null,
    })
  }

  const reorderBooks = async (language, orderedIds) => {
    const updates = orderedIds.map((id, i) =>
      supabase.from('books').update({ sort_order: i + 1 }).eq('id', id)
    )
  ...
  return {
    ...
    addBook, updateBook, deleteBook, toggleOwned, updateProgress, reorderBooks,
    refresh: fetchBooks,
  }
}
